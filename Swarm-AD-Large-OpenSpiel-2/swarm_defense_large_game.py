...
The v2 build enforces the two-wave engagement loop described in the design
notes:

1. Defender places air-defense (AD) units first.
2. Attacker allocates an arbitrary subset of its drones for wave one.
3. Defender reacts with limited interceptors; any drone/AD LOS contact marks the
    AD as discovered for later waves when discovery is required.
4. Remaining drones automatically roll into the second wave where AD targets
    that were discovered become legal strike options.
5. Remaining interceptors fire during wave two; leftover ammo yields a defender
    bonus (`INTERCEPTOR_LEFTOVER_VALUE`).
"""
...
INTERCEPT_SELECTIONS = NUM_ATTACKING_DRONES
...
DRONE_END_ACTION = DRONE_ACTION_BASE + DRONE_ASSIGNMENT_ACTIONS
...
def decode_interceptor_action(action: int) -> int:
    rel = action - INTERCEPT_ACTION_BASE
    if not (0 <= rel < INTERCEPT_SELECTIONS):
...
class SwarmDefenseLargeState(pyspiel.State):
    def __init__(self, game: "SwarmDefenseLargeGame"):
        super().__init__(game)
        self._require_ad_discovery = game.require_ad_discovery
        ...
        self._wave_start_offsets: Dict[int, float] = {}
        self._wave_completion_times: Dict[int, float] = {}
    ...
    def snapshot(self) -> Dict[str, object]:
        return {
            ...
            "remaining_drones": self._remaining_drones,
            "remaining_interceptors": self._remaining_interceptors,
    ...
    def _wave_drone_indices(self, wave: int) -> List[int]:
        return list(self._wave_plan_indices.get(wave, []))

    def _allowed_target_indices_for_wave(self, wave: int) -> set[int]:
        allowed: set[int] = set(range(len(self._targets)))
        max_target_index = len(self._targets) + len(self._ad_units)
        if not self._require_ad_discovery:
            allowed.update(range(len(self._targets), max_target_index))
            return allowed
        if wave > 1:
            for ad_idx, ad_unit in enumerate(self._ad_units):
                if not ad_unit.alive:
                    continue
                if ad_idx in self._discovered_ads:
                    allowed.add(len(self._targets) + ad_idx)
        return allowed

    def _is_target_allowed_for_current_wave(self, target_idx: int) -> bool:
        if target_idx < 0:
            return False
        allowed = self._allowed_target_indices_for_wave(self._current_wave)
        return target_idx in allowed
    ...
    def _available_intercept_targets(self) -> List[int]:
        ...
        for idx in self._wave_drone_indices(self._active_wave):
            ...
            if plan.destroyed_by is not None:
                continue
            if idx in self._interceptor_engaged:
                continue
            choices.append(idx)
        return choices

    def _mark_discoveries_for_wave(self, wave: int) -> None:
        indices = self._wave_plan_indices.get(wave, [])
        if not indices:
            return
        for ad_idx, ad_unit in enumerate(self._ad_units):
            if ad_idx in self._discovered_ads or not ad_unit.alive:
                continue
            position = (ad_unit.row, ad_unit.col)
            for plan_idx in indices:
                plan = self._drone_plans[plan_idx]
                exposure, _, _ = self._path_exposure_stats(plan, position)
                if exposure > 0.0:
                    self._discovered_ads.add(ad_idx)
                    break

    def _complete_current_wave(self) -> None:
        self._mark_discoveries_for_wave(self._current_wave)
        wave_end = self._wave_end_timestamp(self._current_wave)
        self._wave_completion_times[self._current_wave] = wave_end
        self._timeline_cursor = wave_end + WAVE_TRANSITION_BUFFER
        if self._current_wave < NUM_WAVES and self._remaining_drones > 0:
            self._begin_wave(self._current_wave + 1)
            return
        self._phase = Phase.TERMINAL
        self._finalize_returns()

    def _wave_end_timestamp(self, wave: int) -> float:
        indices = self._wave_plan_indices.get(wave, [])
        offset = self._wave_start_offsets.get(wave, self._timeline_cursor)
        if not indices:
            return offset
        times: List[float] = [offset]
        for idx in indices:
            plan = self._drone_plans[idx]
            times.append(plan.arrival_time)
            if plan.interceptor_time is not None:
                times.append(plan.interceptor_time)
            for _, _, intercept_time in plan.intercepts:
    ...
    def legal_actions(self, player: Optional[int] = None) -> List[int]:
        ...
        if self._phase == Phase.SWARM_ASSIGNMENT:
            ...
            if self._wave_quota > 0 and self._wave_drone_count < self._wave_quota:
                for offset in range(DRONE_ASSIGNMENT_ACTIONS):
                    action = DRONE_ACTION_BASE + offset
    ...
    def _apply_drone_action(self, action: int) -> None:
        ...
        if self._remaining_drones <= 0 or self._wave_drone_count >= self._wave_quota:
            raise ValueError("No drones available for deployment")
        entry_idx, target_idx, tot_idx, blueprint_idx = decode_drone_action(action)
        if not self._is_target_allowed_for_current_wave(target_idx):
            raise ValueError("Target not available in this wave")
        max_target_index = len(self._targets) + len(self._ad_units)
        if not (0 <= target_idx < max_target_index):
            raise ValueError("Invalid target specified for drone")
        plan = DronePlan(entry_idx, target_idx, tot_idx, blueprint_idx, wave=self._current_wave)
        self._initialize_drone_path(plan)
        self._drone_plans.append(plan)
        self._wave_plan_indices[self._current_wave].append(len(self._drone_plans) - 1)
        self._wave_drone_count += 1
        self._remaining_drones = max(0, self._remaining_drones - 1)
        if self._remaining_drones == 0 or (
    ...
    def _synchronize_wave_tot_schedule(self, wave: int) -> None:
        ...
        for idx in indices:
            plan = self._drone_plans[idx]
            requirements.append((plan.total_distance / DRONE_SPEED) - LARGE_TOT_CHOICES[plan.tot_idx])
        anchor = max(0.0, *requirements)
        self._tot_anchor = anchor
        wave_offset = self._wave_start_offsets.get(wave, 0.0)
        for idx in indices:
            plan = self._drone_plans[idx]
            ...
            plan.hold_time = wave_offset + hold_time
            plan.arrival_time = wave_offset + hold_time + travel_time
    ...
    def _start_ad_resolution(self) -> None:
        if self._tot_anchor is None:
            self._synchronize_wave_tot_schedule(self._active_wave)
    ...
    def _earliest_drone_targeting_ad(self, ad_idx: int) -> Optional[int]:
        ...
        for idx, plan in enumerate(self._drone_plans):
            if plan.wave != self._active_wave:
                continue
            if plan.destroyed_by is not None:
                continue
    ...
    def _apply_drone_ad_strike(self, action: int) -> None:
        ...
        if success and ad_unit.alive:
            ...
            self._ad_destroy_reward += AD_KILL_REWARD
    ...
    def _finalize_returns(self) -> None:
        total_damage = self._damage_from_targets + self._ad_destroy_reward
        defender_bonus = self._remaining_interceptors * INTERCEPTOR_LEFTOVER_VALUE
        attacker_return = total_damage - defender_bonus
        self._returns = [attacker_return, -attacker_return]
    ...
    def observation_string(self, player: int) -> str:
        ...
        lines.append(f"Wave: {self._current_wave}/{NUM_WAVES}")
        lines.append(
            f"Remaining drones: {self._remaining_drones} | Remaining interceptors: {self._remaining_interceptors}"
        )
        lines.append(f"Discovered ADs: {sorted(self._discovered_ads)}")
        ...
        for idx, plan in enumerate(self._drone_plans):
            ...
            lines.append(
                ...
                f" wave={plan.wave} destroyed_by={plan.destroyed_by}"
            )
    ...
    def action_to_string(self, player: Optional[int], action: int) -> str:
        ...
        if DRONE_ACTION_BASE <= action < DRONE_END_ACTION:
            entry_idx, target_idx, tot_idx, blueprint_idx = decode_drone_action(action)
        ...
        if action == DRONE_END_ACTION:
            return "drone_assign:end_wave"
        if INTERCEPT_ACTION_BASE <= action < INTERCEPT_END_ACTION:
...
_GAME_TYPE = pyspiel.GameType(
    short_name="swarm_defense_large_v2",
    long_name="Swarm Defense Large-Scale Sequential Game v2",
...
_MAX_DAMAGE = (
    NUM_ATTACKING_DRONES * max(TARGET_VALUE_OPTIONS)
    + NUM_INTERCEPTORS * INTERCEPTOR_LEFTOVER_VALUE
    + NUM_AD_UNITS * AD_KILL_REWARD
)
...
_GAME_INFO = pyspiel.GameInfo(
    ...
    max_game_length=(
        ...
        + NUM_ATTACKING_DRONES * NUM_WAVES * 2
    ),
)
...
class SwarmDefenseLargeGame(pyspiel.Game):
    def __init__(self, params: Optional[Dict[str, int]] = None):
        ...
        require_ad_discovery = params.get("require_ad_discovery", 1)
        self._require_ad_discovery = bool(int(require_ad_discovery))
    ...
    @property
    def require_ad_discovery(self) -> bool:
        return self._require_ad_discovery
